...
class TestMLEModel:
    ...
    @given(
        x=arrays(dtype=float, shape=st.integers(min_value=10, max_value=100),
                 elements=st.floats(max_value=1e15, min_value=-1e15,
                  allow_infinity=False, allow_nan=False), unique=True),
        order=st.tuples(st.integers(min_value=1, max_value=3), st.integers(min_value=1, max_value=3))
    )
