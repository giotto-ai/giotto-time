...
def _run_css(
    ...
    if errors:
        ...
        ssr = np.dot(eps, eps)
        sigma2 = ssr / nobs
        loglikelihood = -nobs / 2.0 * (np.log(2 * np.pi * sigma2)) - ssr / (
            2.0 * sigma2
        )
...
class ARMAMLEModel:
    ...
    def fit(self, X: np.array):
        ...
        Xmin = minimize(
            lambda phi: _run_css(phi, X, len_p=self.order[0]),
            x0=self.parameters,
            method="L-BFGS-B",
        )
        ...
        self.theta = _transform_params(
            fitted_params[-self.order[1] :] if self.order[1] > 0 else np.array([])
        )
    ...
    def get_errors(self, X: np.array):
        ...
        """
        errors = _run_css(
            self.parameters, X, self.order[0], errors=True, transform=False
        )
        return errors
